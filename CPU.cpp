...
/* Flags updating */
    inline void upd_cv(uint8_t x, uint8_t y, int16_t r) { P[C] = (r>0xFF); P[V] = ~(x^y) & (x^r) & 0x80; }
    inline void upd_nz(uint8_t x)              { P[N] = x & 0x80; P[Z] = (x == 0);              }
// Does adding I to A cross a page?
    inline bool cross(uint16_t a, uint8_t i) { return ((a+i) & 0xFF00) != ((a & 0xFF00)); }

/* Memory access */
    void dma_oam(uint8_t bank);
    template<bool wr> inline uint8_t access(uint16_t addr, uint8_t v = 0)
    {
        uint8_t* r;
        switch (addr)
        {
           ...
           case 0x4000 ... 0x4013:
            ...
            case            0x4017:
                if (!wr)
                    ...
                    return Joypad::readState(1);                  // Joypad 1.

            case            0x4014:
                if (wr) dma_oam(v); break;                          // OAM DMA.
            case  0x4016:
                if (wr)
                {
                    Joypad::writeStrobe(v & 1);
                    break;
                }     // Joypad strobe.
                else
                    return Joypad::readState(0);                  // Joypad 0.
            case 0x4018 ... 0xFFFF:  return Cartucho::access<wr>(addr, v);              // Cartridge.
        }
        return 0;
    }
    inline uint8_t  wr(uint16_t a, uint8_t v)      { T; return access<1>(a, v);   }
    inline uint8_t  rd(uint16_t a)            { T; return access<0>(a);      }
    ...
    void dma_oam(uint8_t bank) { for (int i = 0; i < 256; i++)  wr(0x2014, rd(bank*0x100 + i)); }

/* Addressing modes */
    inline uint16_t imm()   { return PC++;                                       }
    inline uint16_t imm16() { PC += 2; return PC - 2;                            }
    inline uint16_t abs()   { return rd16(imm16());                              }
    inline uint16_t _abx()  { T; return abs() + X;                               }  // Exception.
    inline uint16_t abx()   { uint16_t a = abs(); if (cross(a, X)) T; return a + X;   }
    inline uint16_t aby()   { uint16_t a = abs(); if (cross(a, Y)) T; return a + Y;   }
    inline uint16_t zp()    { return rd(imm());                                  }
    inline uint16_t zpx()   { T; return (zp() + X) % 0x100;                      }
    inline uint16_t zpy()   { T; return (zp() + Y) % 0x100;                      }
    ...
    inline uint16_t izy()   { uint16_t a = _izy(); if (cross(a-Y, Y)) T; return a;    }

/* STx */
    template<uint8_t& r, Mode m> void st()        {    wr(   m()    , r); }
    template<>              void st<A,izy>() { T; wr(_izy()    , A); }  // Exceptions.
    template<>              void st<A,abx>() { T; wr( abs() + X, A); }  // ...
    template<>              void st<A,aby>() { T; wr( abs() + Y, A); }  // ...

#define G  uint16_t a = m(); uint8_t p = rd(a)  /* Fetch parameter */
    template<uint8_t& r, Mode m> void ld()  { G; upd_nz(r = p);                  }  // LDx
    template<uint8_t& r, Mode m> void cmp() { G; upd_nz(r - p); P[C] = (r >= p); }  // CMP, CPx
/* Arithmetic and bitwise */
    template<Mode m> void ADC() { G       ; int16_t r = A + p + P[C]; upd_cv(A, p, r); upd_nz(A = r); }
    template<Mode m> void SBC() { G ^ 0xFF; int16_t r = A + p + P[C]; upd_cv(A, p, r); upd_nz(A = r); }
...
/* Read-Modify-Write */
    ...
    template<Mode m> void ROL() { G; uint8_t c = P[C]     ; P[C] = p & 0x80; T; upd_nz(wr(a, (p << 1) | c) ); }
    template<Mode m> void ROR() { G; uint8_t c = P[C] << 7; P[C] = p & 0x01; T; upd_nz(wr(a, c | (p >> 1)) ); }
    template<Mode m> void DEC() { G; T; upd_nz(wr(a, --p)); }
    template<Mode m> void INC() { G; T; upd_nz(wr(a, ++p)); }
#undef G

/* DEx, INx */
    template<uint8_t& r> void dec() { upd_nz(--r); T; }
    template<uint8_t& r> void inc() { upd_nz(++r); T; }
/* Bit shifting on the accumulator */
    void ASL_A() { P[C] = A & 0x80; upd_nz(A <<= 1); T; }
    void LSR_A() { P[C] = A & 0x01; upd_nz(A >>= 1); T; }
    void ROL_A() { uint8_t c = P[C]     ; P[C] = A & 0x80; upd_nz(A = ((A << 1) | c) ); T; }
    void ROR_A() { uint8_t c = P[C] << 7; P[C] = A & 0x01; upd_nz(A = (c | (A >> 1)) ); T; }

/* Txx (move values between registers) */
    template<uint8_t& s, uint8_t& d> void tr()      { upd_nz(d = s); T; }
...
/* Flow control (branches, jumps) */
    template<Flag f, bool v> void br() { int8_t j = rd(imm()); if (P[f] == v) { T; PC += j; } }
    void JMP_IND() { uint16_t i = rd16(imm16()); PC = rd16_d(i, (i&0xFF00) | ((i+1) % 0x100)); }
    void JMP()     { PC = rd16(imm16()); }
    void JSR()     { uint16_t t = PC+1; T; push(t >> 8); push(t); PC = rd16(imm16()); }
